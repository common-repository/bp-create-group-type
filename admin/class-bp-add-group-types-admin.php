...
if ( ! class_exists( 'Bp_Add_Group_Types_Admin' ) ) :
	...
	class Bp_Add_Group_Types_Admin {
		...
		public function enqueue_scripts() {
			...
			// }
			if ( 'wb-plugins_page_bp-add-group-types' === $screen->base ) {
				wp_enqueue_style( 'font-awesome-css', 'http://use.fontawesome.com/releases/v5.5.0/css/all.css?', array(), '5.3.2', 'all' );
				wp_enqueue_style( $this->plugin_name . '-css', plugin_dir_url( __FILE__ ) . 'css/bp-add-group-types-admin.css', array(), $this->version, 'all' );
				wp_enqueue_script( $this->plugin_name . '-js', plugin_dir_url( __FILE__ ) . 'js/bp-add-group-types-admin.js', array( 'jquery' ), $this->version, false );
			}

			if ( isset( $screen->post_type ) && $this->post_type === $screen->post_type || ( isset( $screen->taxonomy ) && 'bp_group_type' == $screen->taxonomy ) ) {
			...
			wp_localize_script(
				...
				array(
					...
					'ajax_nonce' => wp_create_nonce( 'bpgt-nonce' ),
				)
			);
		}
			...
			<div class="wbcom-wrap wbcom-plugin-wrapper">
				...
				<div class="wbcom_admin_header-wrapper">
					<div id="wb_admin_plugin_name">
						<?php esc_html_e( 'BuddyPress Group Types', 'bp-add-group-types' ); ?>
						<span><?php printf( __( 'Version %s', 'bp-add-group-types' ), BP_GROUP_TYPE_PLUGIN_VERSION ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped?></span>
					</div>
		...
		public function ajax_check_type_id() {
			if ( isset( $_POST['nonce'] ) && ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'bpgt-nonce' ) ) {
				exit();
			}
		...
		public function save_meta_fields( $post_id, $fields = array() ) {
			...
			if ( empty( $_POST['singular_name'] ) ) {
				$_POST['singular_name'] = sanitize_text_field( wp_unslash( $_POST['post_title'] ) );
			}
			if ( empty( $_POST['type_id'] ) ) {
				$_POST['type_id'] = sanitize_text_field( wp_unslash( $_POST['singular_name'] ) );
			}
			// Sanitize the type_id and custom directory slug.
			$_POST['type_id'] = sanitize_title( wp_unslash( $_POST['type_id'] ) );
			if ( isset( $_POST['has_directory_slug'] ) && sanitize_text_field( wp_unslash( $_POST['has_directory_slug'] ) ) ) {
				$_POST['has_directory_slug'] = isset( $_POST['has_directory_slug'] ) ? sanitize_title( wp_unslash( $_POST['has_directory_slug'] ) ) : '';
			}
			$_POST['type_id'] = sanitize_title( wp_unslash( $_POST['type_id'] ) );
			foreach ( $fields as $field ) {
				$old_setting = get_post_meta( $post_id, $field, true );
				$new_setting = ( isset( $_POST[ $field ] ) ) ? sanitize_title( wp_unslash( $_POST[ $field ] ) ) : '';
				$success     = false;
		...
		public function ajax_check_type_id_send_response() {
			...
			if ( ! isset( $_POST['type'] ) ) {
				if ( ! isset( $_POST['singular_name'] ) ) {
					...
					$type = sanitize_title( wp_unslash( $_POST['singular_name'] ) );
				}
			} else {
				$type = sanitize_title( wp_unslash( $_POST['type'] ) );
			}

			if ( isset( $_POST['pagenow'] ) && 'bp_member_type' == $_POST['pagenow'] ) {
				if ( null == bp_get_member_type_object( $type ) ) {
					wp_send_json_success( esc_html__( 'Type ID is unique.', 'bp-add-group-types' ) );
				}
			} elseif ( isset( $_POST['pagenow'] ) && 'bp_group_type' == $_POST['pagenow'] ) {
		 ...
		 * @since 2.0.0
		 *
		 * @param WP_Post $post_id Posts Object.
		 * @return mixed|string
		 */
		...
		/**
		 * Group type taxonomy labels.
		 *
		 * @param  array $group_type_fields Associative array (name => label).
		 * @return array
		 */
		...
		/**
		 * Add new meta to the BuddyPress Group type metadata.
		 *
		...
		/**
		 * BuddyPress group type edit form.
		 *
		 * @param  array  $term Terms Object.
		 * @param  string $taxonomy The Type's taxonomy name.
		 * @return void
		 */
		 ...
		 * @return void
		 */
		public function bp_create_groups_type_hide_all_admin_notices_from_setting_page() {
			$wbcom_pages_array  = array( 'wbcomplugins', 'wbcom-plugins-page', 'wbcom-support-page', 'bp-add-group-types' );
